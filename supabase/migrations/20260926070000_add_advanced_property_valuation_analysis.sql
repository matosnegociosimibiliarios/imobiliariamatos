...
declare
  ...
  v_rent numeric;
  v_estimated numeric;
  v_mean_m2 numeric; v_median_m2 numeric; v_min_m2 numeric; v_max_m2 numeric; v_stddev_m2 numeric;
  v_cv numeric; v_count integer := 0; v_closed_count integer := 0; v_listing_count integer := 0;
  v_avg_similarity numeric; v_avg_distance numeric; v_avg_dom numeric; v_median_dom numeric; v_avg_discount numeric; v_gross_yield numeric; v_market_position numeric;
begin
  ...
  with accepted as (
    ...
    select count(*)::integer count,
      ...
      avg(price_m2) mean_m2, percentile_cont(0.5) within group(order by price_m2) median_m2,
      ...
      avg(days_on_market) filter(where days_on_market is not null) avg_dom,
      percentile_cont(0.5) within group(order by days_on_market) filter(where days_on_market is not null) median_dom,
  ...
  select count,closed_count,listing_count,mean_m2,median_m2,min_m2,max_m2,stddev_m2,avg_similarity,avg_distance,avg_dom,median_dom,avg_discount
  into v_count,v_closed_count,v_listing_count,v_mean_m2,v_median_m2,v_min_m2,v_max_m2,v_stddev_m2,v_avg_similarity,v_avg_distance,v_avg_dom,v_median_dom,v_avg_discount from stats;
  v_cv:=case when coalesce(v_mean_m2,0)>0 then coalesce(v_stddev_m2,0)/v_mean_m2*100 end;
  v_gross_yield:=case when coalesce(v_rent,0)>0 and coalesce(v_estimated,0)>0 then (v_rent*12/v_estimated)*100 end;
  v_market_position:=case when coalesce(v_estimated,0)>0 and coalesce(v_median_m2,0)>0 and coalesce(v_area,0)>0 then (v_estimated/(v_median_m2*v_area)-1)*100 end;
  return jsonb_build_object(
    'sample',jsonb_build_object('count',coalesce(v_count,0),'closed_sales',coalesce(v_closed_count,0),'active_listings',coalesce(v_listing_count,0),'avg_similarity',round(coalesce(v_avg_similarity,0),2),'avg_distance_km',case when v_avg_distance is null then null else round(v_avg_distance,2) end),
    'price_per_m2',jsonb_build_object('mean',case when v_mean_m2 is null then null else round(v_mean_m2,2) end,'median',case when v_median_m2 is null then null else round(v_median_m2,2) end,'minimum',case when v_min_m2 is null then null else round(v_min_m2,2) end,'maximum',case when v_max_m2 is null then null else round(v_max_m2,2) end,'stddev',case when v_stddev_m2 is null then null else round(v_stddev_m2,2) end,'coefficient_variation_pct',case when v_cv is null then null else round(v_cv,2) end),
